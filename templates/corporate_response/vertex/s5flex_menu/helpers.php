			...
			if(!empty($parent_items)){
				...
				foreach($parent_items as $prow => $pitem){
					...
					if ($iParams->get('menu_image') && $iParams->get('menu_image') != -1) {
						...
						$image = '<span class="s5_img_span"><img style="cursor:pointer" src="'.$iParams->get('menu_image').'" onclick="window.document.location.href=\''.$pitem->url.'\'" '.$imgalign.' alt="'.$pitem->alias.'" /></span>';
						/*if($tmp->ionly){
							 $tmp->name = null;
						 }*/
					} else {
...
/*	function getChildItems($parent_id, $child_items, $maxdepth, $current_depth, $parent_item_columns){
	...
	function getMenuContent($S5row){
		...
		if ($iParams->get('menu_image') && $iParams->get('menu_image') != -1) {
			...
			$image = '<span class="s5_img_span"><img style="cursor:pointer" src="'.$iParams->get('menu_image').'" onclick="window.document.location.href=\''.$S5row->url.'\'" '.$imgalign.' alt="'.$S5row->alias.'" /></span>';
			/*if($tmp->ionly){
				 $tmp->name = null;
			 }*/
		} else {
